...
async def on_message(message):
    ...
    if message.content.startswith("start Weerwolven") or message.content.startswith("Start Weerwolven"):
        global main_channel
        main_channel = client.get_channel(message.channel.id)
        await main_channel.send("In dit channel kan er nu Weerwolven worden gespeeld!")
        await main_channel.send("Wie doet er mee met Weerwolven?")
...
def role_selector():
    # dictionary, zodat bij elke rol aantal kaarten bij staan.
    roles = {"Weerwolf": 4,
             "Burger": 13,
             "Ziener": 1,
             "Heks": 1,
